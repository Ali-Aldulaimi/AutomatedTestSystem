...
Power On DUT and Check Other Devices
  Power On/Off DUT    255        #pass only the numerical part of DUT name ex:AQ-T254 = 254
...
Pre06: DBG_PROF_TOTAL around 1100 (+/-100)
    FOR  ${index}  IN RANGE  1  10
...
Pre07: DBG_PROF_FMT_PREPARATION around 55 (+/-5)
    FOR  ${index}  IN RANGE  1  10
        ${result} =	Run Process	 //ARCDT023/artifacts//AQconsole(exe)/AQconsole.exe  dbr  DBG_PROF_FMT_PREPARATION  192.168.100.53  25  40  timeout=10s
        Log   ${result.stderr}
        ${bStatus} =  Run Keyword And Return Status  Should Contain	 ${result.stderr}	Compare:OK
        Exit For Loop If  ${bStatus} == ${True}
    END
...
Pre08: DBG_PROF_FMT_FFT around 90 (+/-5)
    FOR  ${index}  IN RANGE  1  10
        ${result} =	Run Process	 //ARCDT023/artifacts//AQconsole(exe)/AQconsole.exe  dbr  DBG_PROF_FMT_FFT  192.168.100.53  40  60  timeout=10s
        Log   ${result.stderr}
        ${bStatus} =  Run Keyword And Return Status  Should Contain	 ${result.stderr}	Compare:OK
        Exit For Loop If  ${bStatus} == ${True}
    END
...
Pre9: DBG_PROF_FMT_MAG_ANGLE around 145 (+/-10)
    FOR  ${index}  IN RANGE  1  10
        ${result} =	Run Process	 //ARCDT023/artifacts//AQconsole(exe)/AQconsole.exe  dbr  DBG_PROF_FMT_MAG_ANGLE  192.168.100.53  80  90  timeout=10s
        Log   ${result.stderr}
         ${bStatus} =  Run Keyword And Return Status  Should Contain	 ${result.stderr}	Compare:OK
        Exit For Loop If  ${bStatus} == ${True}
    END
...
Pre10: DBG_PROF_FMT_FREQ_ORTO around 65 (+/-10)
    FOR  ${index}  IN RANGE  1  10
        ${result} =	Run Process	 //ARCDT023/artifacts//AQconsole(exe)/AQconsole.exe  dbr  DBG_PROF_FMT_FREQ_ORTO  192.168.100.53  160  180  timeout=10s
        Log   ${result.stderr}
         ${bStatus} =  Run Keyword And Return Status  Should Contain	 ${result.stderr}	Compare:OK
        Exit For Loop If  ${bStatus} == ${True}
    END
...
Pre11: DBG_PROF_LOGIC_EXECUTE around 5 (+/-2)
    FOR  ${index}  IN RANGE  1  10
...
Pre12: DBG_PROF_MATLAB_MEAS around 400 (+/-30)
    FOR  ${index}  IN RANGE  1  10
        ${result} =	Run Process	 //ARCDT023/artifacts//AQconsole(exe)/AQconsole.exe  dbr  DBG_PROF_MATLAB_MEAS  192.168.100.53  330  400  timeout=10s
        Log   ${result.stderr}
         ${bStatus} =  Run Keyword And Return Status  Should Contain	 ${result.stderr}	Compare:OK
        Exit For Loop If  ${bStatus} == ${True}
    END
...
Pre13: DBG_PROF_MATLAB_STEP around 350 (+/-100)
    FOR  ${index}  IN RANGE  1  10
...
Pre14.1: All DIs are "Not active" in the beginning
...
pre14.2: All DIs are "Not active" in the beginning (LabJack)
     Check All DIs Are Not Active

Pre15.1: set T1 to "NC"
...
Pre20.1 system fault or is powered OFF. (SF-pin 16-17)
    Check Input State for Labjack    2    17    1        #LJnum    InputPin    expectedValue
                                                         #Pins 16 and 17 are closed when the unit has a system fault or is powered OFF
       

Pre20.2 the unit is powered ON and there is no system fault. (SF-pin 16-18 "NO")
...
Pre21: Apply 24V on DIs (Activaite DO.2.1-DO.2.3 )
	Apply 24V on DIs

Pre22.1: DIs ??&?? are "Active"
    Sleep  3s  Give some time for the DIs to settle
    ${result} =	Run Process	 //ARCDT023/artifacts//AQconsole(exe)/AQconsole.exe  dbr  DIGI_DI1_32  192.168.100.53  0  0  timeout=10s
    Log   ${result.stderr}
    Should Contain	 ${result.stderr}	DIGI_DI1_32 value 7

Pre22.2: Remove 24V from DIs (DEActivaite DO.2.1-DO.2.3 )
	
        Remove 24V from DIs

Pre23: Set T1 polarity to "NO"
...
Pre32.2: T5 polarity is "NO"
     Check Input State for Labjack    2    16    1        #LJnum    InputPin    expectedValue
     
Pre33.1: All DIs are "Not active" at the end
    Sleep  3s  Give some time for the DIs to settle
    ${result} =	Run Process	 //ARCDT023/artifacts//AQconsole(exe)/AQconsole.exe  dbr  DIGI_DI1_32  192.168.100.53  0  0  timeout=10s
    Log   ${result.stderr}
    Should Contain	 ${result.stderr}	DIGI_DI1_32 value 0

pre33.2: All DIs are "Not active" at the end
    Check All DIs Are Not Active
