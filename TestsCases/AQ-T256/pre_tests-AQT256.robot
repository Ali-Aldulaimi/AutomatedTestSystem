...
Power On DUT and Check Other Devices
    Power On/Off DUT    256        #pass only the numerical part of DUT name ex:AQ-T254 = 254
...
Pre09: DBG_PROF_FMT_MAG_ANGLE around 145 (+/-10)
    FOR  ${index}  IN RANGE  1  10
...
Pre10: DBG_PROF_FMT_FREQ_ORTO around 65 (+/-10)
    FOR  ${index}  IN RANGE  1  10
        ${result} =	Run Process	 //ARCDT023/artifacts//AQconsole(exe)/AQconsole.exe  dbr  DBG_PROF_FMT_FREQ_ORTO  192.168.100.53  140  350  timeout=10s
        Log   ${result.stderr}
         ${bStatus} =  Run Keyword And Return Status  Should Contain	 ${result.stderr}	Compare:OK
        Exit For Loop If  ${bStatus} == ${True}
    END
...
Pre20 system fault or is powered OFF. (SF-pin 16-17)
...
Pre21 the unit is powered ON and there is no system fault. (SF-pin 16-18 "NO") 
...
Pre23: DIs ??&?? are "Active"
    Sleep  3s  Give some time for the DIs to settle
    ${result} =	Run Process	 //ARCDT023/artifacts//AQconsole(exe)/AQconsole.exe  dbr  DIGI_DI1_32  192.168.100.53  0  0  timeout=10s
    Log   ${result.stderr}
    Should Contain	 ${result.stderr}	DIGI_DI1_32 value 7

Pre24: Remove 24V from DIs (DEActivaite DO.2.1-DO.2.3 )
	
        Remove 24V from DIs

Pre25: Set T1 polarity to "NO"
